...
if (isset($_POST['save_results'])) {
    // Loop through the submitted students and their marks
    foreach ($_POST['ien_number'] as $key => $ien_number) {
        $TW_marks = $_POST['TW_marks'][$key] ?? null;
        $PR_marks = $_POST['PR_marks'][$key] ?? null;
        $ESE_marks = $_POST['ESE_marks'][$key] ?? null;

        // Check if the student exists in the results table
        $checkQuery = "SELECT * FROM results WHERE ien_number = '$ien_number'";
        $checkResult = $conn->query($checkQuery);

        if ($checkResult->num_rows > 0) {
            // Student exists, update the record
            $updateQuery = "UPDATE results SET ";
            if ($TW_marks !== null) $updateQuery .= "TW_marks = '$TW_marks', ";
            if ($PR_marks !== null) $updateQuery .= "PR_marks = '$PR_marks', ";
            if ($ESE_marks !== null) $updateQuery .= "ESE_marks = '$ESE_marks' ";
            $updateQuery .= "WHERE ien_number = '$ien_number'";
            $conn->query($updateQuery);

        } else {
            // Student doesn't exist, insert a new record
            $insertQuery = "INSERT INTO results (ien_number, subject, TW_marks, PR_marks, ESE_marks, year, batch, semester, department) VALUES ('$ien_number', ";
            $insertQuery .= $TW_marks !== null ? "'$TW_marks', " : "NULL, ";
            $insertQuery .= $PR_marks !== null ? "'$PR_marks', " : "NULL, ";
            $insertQuery .= $ESE_marks !== null ? "'$ESE_marks')" : "NULL )";
...
<body>
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            foreach ($subjects as $subject) {
                                $sanitized_subject = sanitize_subject_name($subject['Subject']);
                                if ($subject['tw_checked']) echo "<td class='subject-column subject-$sanitized_subject'><input type='number' name='TW_marks[]'></td>";
                                if ($subject['pr_checked']) echo "<td class='subject-column subject-$sanitized_subject'><input type='number' name='PR_marks[]'></td>";
                                if ($subject['ese_checked']) echo "<td class='subject-column subject-$sanitized_subject'><input type='number' name='ESE_marks[]'></td>";
                            }
                            echo "</tr>";
                        }
                    } else {
    ...
    <script>
        function toggleSubjectColumn(subject) {
            // Sanitize subject string in JavaScript (spaces, colons, etc.)
            console.log('Toggling subject column:', subject);
            const subjectElements = document.querySelectorAll('.subject-' + subject);
            console.log(subjectElements);

            subjectElements.forEach(function(element) {
                if (element.style.display === 'none' || element.style.display === '') {
                    element.style.display = 'table-cell'; // Show the column
                } else {
                    element.style.display = 'none'; // Hide the column
                }
            });
        }
